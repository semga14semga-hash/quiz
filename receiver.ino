...
struct Packet {
  byte type;        // 1 = вопрос, 2 = вариант, 3 = ответ, 4 = подтверждение вопроса, 5 = конец опроса
  byte sender;      // ID отправителя (1-8 для пультов)
  byte qNum;        // Номер вопроса
  byte part;
  byte last;
  char text[30];
};
...
void sendChunk(byte type, byte sender, byte qNum, byte part, bool last, const char* data) {
  packet.type = type;
  ...
  delay(3);
  radio.write(&packet, sizeof(packet)); // Повтор
  switchToRx();

  delay(8);
}

void sendText(byte type, byte sender, byte qNum, String txt) {
  int len = txt.length();
  byte part = 0;
  ...
  for (int i = 0; i < len; i += 29) {
    ...
    bool last = (i + 29 >= len);
    sendChunk(type, sender, qNum, part, last, chunk);
...
void sendQuiz(uint8_t qNum, String q, String a1, String a2, String a3, String a4) {
  ...
  // Отправляем всем (sender = 0 для "всем")
  sendText(1, 0, qNum, q);
  delay(80);
  sendText(2, 0, qNum, a1);
  delay(80);
  sendText(2, 0, qNum, a2);
  delay(80);
  sendText(2, 0, qNum, a3);
  delay(80);
  sendText(2, 0, qNum, a4);
